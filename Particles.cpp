...
float time; // The current time of the particle system
...
{
	particles_count++;
	particle p = particle{ particles_count, 1.0, time + PART_LIFE_SPAN, {PART_START_POS[0], PART_START_POS[1] , PART_START_POS[2] }, { generateSpeed(), generateSpeed(), generateSpeed() } };
...
void displayParticles(void)
{
	...
	for (auto it = particles.begin(); it < particles.end(); ++it)
	{
		...
		float red = 255 * (p.expire_time - time) / PART_LIFE_SPAN;
...
void menu(int id) {
	if (id == 1) // Toggle particle generation
	{
		isGenerating = !isGenerating;
	}
	else if (id == 2) // Toggle particles affection by wind
	{
		hasWind = !hasWind;
	}
	else { // Quit
...
void main(int argc, char** argv)
{
	...
	// Create menu
	glutCreateMenu(menu);
	glutAddMenuEntry("Toggle Generator", 1);
	glutAddMenuEntry("Toggle Wind", 2);
